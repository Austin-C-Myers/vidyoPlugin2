...
public class VidyoSampleActivity extends Activity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(fakeR.getId("layout", "conference"));
		...
		// Hook-up exit button
		final Button btnEnd = (Button) findViewById(fakeR.getId("id", "button_cancel"));
		...
		final View view = (View) findViewById(fakeR.getId("id", "engagement_layout"));
		view.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				confirmCancel = false;
				btnEnd.setText(fakeR.getId("values", "misc_cancel"));
			}	
		});
	...
	private void confirmCancelButton() {
		...
		if (confirmCancel) {
			...
			confirmCancel = true;
			btnEnd.setText(fakeR.getId("values", "confirm_cancel"));
		}
	}
	...
	private void refreshVideo() {
		...
		final RelativeLayout layout = (RelativeLayout) findViewById(fakeR.getId("id", "refresh_video_content"));
	...
	private void switchToWaitingRoom() {
		final TextView message = (TextView) findViewById(fakeR.getId("id", "just_relax"));
		message.setText(String.format(getString(fakeR.getId("values", "be_there_shortly"))));
		
		final ImageView image = (ImageView) findViewById(fakeR.getId("id", "connecting_image"));
		//image.setImageResource(R.drawable.img_video_engagement_waiting);
	}

	private void switchToVideoView() {
		
		final ViewFlipper vf = (ViewFlipper) findViewById(fakeR.getId("id", "ViewFlipper01"));
		vf.showNext();

		final RelativeLayout layout = (RelativeLayout) findViewById(fakeR.getId("id", "engagement_layout"));
		final RelativeLayout layout2 = (RelativeLayout) findViewById(fakeR.getId("id", "engagement_layout2"));
		...
		endBtn = (Button) findViewById(fakeR.getId("id", "button_end"));
		...
		refreshVideoBtn = (Button) findViewById(fakeR.getId("id", "button_refresh_video"));
		refreshVideoBtn.setEnabled(false);
	...
	private void sendEndEngagement() {
		final RelativeLayout wrapUpLayout = (RelativeLayout) findViewById(fakeR.getId("id", "wrapup_content"));
	...
	private void onEngagementStatus() {
		...
		final RelativeLayout refreshVideoLayout = (RelativeLayout) findViewById(fakeR.getId("id", "refresh_video_content"));
...
//		LmiDeviceManagerViewResize(display.getWidth(), display.getHeight());
	...
	public void onVidyoJoinConferenceUpdated() {
		...
		final RelativeLayout textContent = (RelativeLayout) findViewById(fakeR.getId("id", "engagement_text_content"));
